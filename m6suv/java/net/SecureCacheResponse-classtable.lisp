; SecureCacheResponse-classtable.lisp
; Automatically generated by jvm2acl2 on Tue May 27 00:39:37 CDT 2014.
;

(defconst *java.net.SecureCacheResponse*
 (make-class-def
      '(class "java.net.SecureCacheResponse"
            "java.net.CacheResponse"
            (constant_pool)
            (fields)
            (methods
                        (method "<init>"
                              (parameters )
                              (returntype . void)
                              (accessflags  *class*  *public* )
                              (code
                                   (max_stack . 1) (max_locals . 1) (code_length . 5)
                                   (parsedcode
                                      (0 (aload_0))
                                      (1 (invokespecial
					(methodCP "<init>" "java.net.CacheResponse" () void)))
                                      (4 (return))
                                      (endofcode 5))
                                   (Exceptions )
                                   (StackMap )))
                        (method "getCipherSuite"
                              (parameters )
                              (returntype . (class "java.lang.String"))
                              (accessflags  *abstract*  *class*  *public* )
                              (code))
                        (method "getLocalCertificateChain"
                              (parameters )
                              (returntype . (class "java.util.List"))
                              (accessflags  *abstract*  *class*  *public* )
                              (code))
                        (method "getServerCertificateChain"
                              (parameters )
                              (returntype . (class "java.util.List"))
                              (accessflags  *abstract*  *class*  *public* )
                              (code))
                        (method "getPeerPrincipal"
                              (parameters )
                              (returntype . (class "java.security.Principal"))
                              (accessflags  *abstract*  *class*  *public* )
                              (code))
                        (method "getLocalPrincipal"
                              (parameters )
                              (returntype . (class "java.security.Principal"))
                              (accessflags  *abstract*  *class*  *public* )
                              (code)))
            (interfaces)
            (accessflags  *abstract*  *class*  *public*  *super*  *synchronized* )
            (attributes
              (attribute "SourceFile")))))


(defconst *SecureCacheResponse-class-table*
  (make-static-class-decls 
   *java.net.SecureCacheResponse*))

(defconst *package-name-map* 
  ("java.net.SecureCacheResponse" . "java.net"))

